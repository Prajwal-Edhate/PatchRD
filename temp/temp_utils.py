...
def get_vox_from_binvox(objname):
    ...
    voxel_model_512 = binvox_rw.read_as_3d_array(voxel_model_file, fix_coords=True).data.astype(cp.uint8)
    step_size = 2
    voxel_model_256 = voxel_model_512[0::step_size,0::step_size,0::step_size]
    for i in range(step_size):
        for j in range(step_size):
            for k in range(step_size):
                voxel_model_256 = cp.maximum(voxel_model_256,voxel_model_512[i::step_size,j::step_size,k::step_size])
...
def get_vox_from_binvox_1over2(objname):
    ...
    #get voxel models
    voxel_model_file = open(objname, 'rb')
    voxel_model_512 = cp.asarray(binvox_rw.read_as_3d_array(voxel_model_file, fix_coords=True).data.astype(np.uint8))
    step_size = 4
    ...
    for i in range(step_size):
        for j in range(step_size):
            for k in range(step_size):
                voxel_model_128 = cp.asarray(np.maximum(voxel_model_128,voxel_model_512[i::step_size,j::step_size,k::step_size]))
    #add flip&transpose to convert coord from shapenet_v1 to shapenet_v2
    #voxel_model_128 = np.flip(np.transpose(voxel_model_128, (2,1,0)),2)
    voxel_model_256 = cp.zeros([256,256,256],cp.uint8)
...
def get_vox_from_binvox_1over2_return_small(objname):
    ...
    #get voxel models
    voxel_model_file = open(objname, 'rb')
    voxel_model_512 = cp.asarray(binvox_rw.read_as_3d_array(voxel_model_file, fix_coords=True).data.astype(cp.uint8))
    step_size = 4
    padding_size = 256%step_size
    output_padding = 128-(256//step_size)
    #voxel_model_512 = voxel_model_512[padding_size:-padding_size,padding_size:-padding_size,padding_size:-padding_size]
    ...
    for i in range(step_size):
        for j in range(step_size):
            for k in range(step_size):
                voxel_model_128 = cp.maximum(voxel_model_128,voxel_model_512[i::step_size,j::step_size,k::step_size])
    #add flip&transpose to convert coord from shapenet_v1 to shapenet_v2
    #voxel_model_128 = np.flip(np.transpose(voxel_model_128, (2,1,0)),2)
...
class voxel_renderer:
    def __init__(self, render_IO_vox_size=256, render_boundary_padding_size=16):
        ...
        self.voxel_idxs = cp.linspace(-self.render_fix_vox_size/2+0.5, self.render_fix_vox_size/2-0.5, self.render_fix_vox_size, dtype = cp.float32)
        self.voxel_x, self.voxel_y, self.voxel_z = cp.meshgrid(self.voxel_idxs,self.voxel_idxs,self.voxel_idxs, sparse=False, indexing='ij')
    ...
    def render_img(self, voxel_in, threshold, view=0, get_depth=False, ray_x = 0, ray_y = 0, ray_z = 1, steep_threshold = 16):
        ...
        voxel = cp.zeros([self.render_fix_vox_size,self.render_fix_vox_size,self.render_fix_vox_size], cp.uint8)
        voxel[self.render_boundary_padding_size:-self.render_boundary_padding_size,self.render_boundary_padding_size:-self.render_boundary_padding_size,self.render_boundary_padding_size:-self.render_boundary_padding_size] = (voxel_in>threshold).astype(cp.uint8)[::-1,::-1]
        ...
        if view==0: #x-y-z
            ...
            new_x = cp.clip(new_x.astype(cp.int32), 0, self.render_fix_vox_size-1)
            new_y = cp.clip(new_y.astype(cp.int32), 0, self.render_fix_vox_size-1)
            new_z = cp.clip(new_z.astype(cp.int32), 0, self.render_fix_vox_size-1)
            voxel = voxel[new_x,new_y,new_z]
            mask = cp.amax(voxel, axis=0).astype(cp.int32)
            depth = cp.argmax(voxel,axis=0)
        if view==1: #right-top
        ...
        if view==2: #front-top
            ...
            new_z = cp.clip(new_z.astype(cp.int32), 0, self.render_fix_vox_size-1)
            voxel = voxel[new_x,new_y,new_z]
            mask = cp.amax(voxel, axis=2).astype(cp.int32)
            depth = cp.argmax(voxel,axis=2)
            mask = cp.flip(cp.transpose(mask, (1,0)),0)
            depth = cp.flip(cp.transpose(depth, (1,0)),0)
        ...
        if view==4: #left
            ...
            depth = cp.argmax(voxel,axis=0)
        if view==5: #top
            mask = cp.amax(voxel, axis=1).astype(cp.int32)
            depth = cp.argmax(voxel,axis=1)
            mask = cp.transpose(mask, (1,0))
            depth = cp.transpose(depth, (1,0))
        if view==6: #front
            mask = cp.amax(voxel, axis=2).astype(cp.int32)
            depth = cp.argmax(voxel,axis=2)
            mask = cp.transpose(mask, (1,0))
        ...
        if get_depth: #depth
            output = 255 + cp.min(depth) - depth
            output = cp.clip(output, 0,255).astype(cp.uint8)
        else: #surface
            output = cp.ones([self.render_fix_vox_size,self.render_fix_vox_size],cp.float32)
            dx = depth[:,:-1] - depth[:,1:]
            dy = depth[:-1,:] - depth[1:,:]
            dxp = 0
            dyp = 0
            ...
            for iii in range(12):
                if iii==0: #/\ 12 25
                    ...
                    partial_dx = dx[1:-1,:-1]
                    partial_dy = dy[1:,:-2]
                partial_m = (cp.abs(partial_dx)<steep_threshold) & (cp.abs(partial_dy)<steep_threshold)
                dxp = dxp+partial_dx*partial_m
                dyp = dyp+partial_dy*partial_m
                counter = counter+partial_m

            counter = cp.maximum(counter,1)
            dxp = dxp/counter
            dyp = dyp/counter

            ds = cp.sqrt(dxp**2 + dyp**2 + 1)
            dxp = dxp/ds
            dyp = dyp/ds
            dzp = 1.0/ds
    ...
    def render_img_with_camera_pose(self, voxel_in, threshold, cam_alpha = 0.785, cam_beta = 0.785, get_depth = False, processed = False, ray_x = 0, ray_y = 0, ray_z = 1, steep_threshold = 16):
        ...
        if processed:
            if self.render_fix_vox_size!=imgsize:
                ...
                exit(-1)
            voxel = (voxel_in>threshold).astype(cp.uint8)
        else:
            if self.render_IO_vox_size!=imgsize:
                print("ERROR: render_img() voxel size does not match!", imgsize, self.render_IO_vox_size)
                exit(-1)
        ...
        #get mask and depth
        sin_alpha = cp.sin(cam_alpha)
        cos_alpha = cp.cos(cam_alpha)
        sin_beta = cp.sin(cam_beta)
        cos_beta = cp.cos(cam_beta)
    ...
    def render_img_with_camera_pose_gpu(self, voxel_in, threshold, cam_alpha = 0.785, cam_beta = 0.785, get_depth = False, processed = False, ray_x = 0, ray_y = 0, ray_z = 1, steep_threshold = 16):
        if processed:
            ...
            if self.render_IO_vox_size!=imgsize:
                print("ERROR: render_img() voxel size does not match!", imgsize, self.render_IO_vox_size)
                exit(-1)
            voxel = cp.zeros([self.render_fix_vox_size,self.render_fix_vox_size,self.render_fix_vox_size], cp.float32)
            ...
            voxel_tensor = torch.from_numpy(cp.asnumpy(voxel)).to(self.device).unsqueeze(0).unsqueeze(0).float()
        ...
        new_x_tensor = cp.asnumpy(new_x_tensor)
        new_y_tensor = cp.asnumpy(new_y_tensor)
        new_z_tensor = cp.asnumpy(new_z_tensor)
        ...
        if get_depth: #depth
            ...
            output = cp.asarray(output.detach()).astype(cp.uint8)
        else: #surface
            dx = depth[:,:-1] - depth[:,1:]
            dy = depth[:-1,:] - depth[1:,:]
            dxp = 0
