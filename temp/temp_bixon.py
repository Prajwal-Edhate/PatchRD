...
def read_as_3d_array(fp, fix_coords=True):
    ...
    raw_data = cp.asarray(np.frombuffer(fp.read(), dtype=np.uint8))
    ...
    data = cp.repeat(values.get(), counts.get()).astype(bool)
...
def read_as_coord_array(fp, fix_coords=True):
    ...
    sz = cp.prod(dims)
    index, end_index = 0, 0
    end_indices = cp.cumsum(counts)
    indices = cp.concatenate(([0], end_indices[:-1])).astype(end_indices.dtype)
    ...
    y = zwpy % dims[0]
    if fix_coords:
        data = cp.vstack((x, y, z))
        axis_order = 'xyz'
    else:
        data = cp.vstack((x, z, y))
        axis_order = 'xzy'

    #return Voxels(data, dims, translate, scale, axis_order)
    return Voxels(cp.ascontiguousarray(data), dims, translate, scale, axis_order)
...
def dense_to_sparse(voxel_data, dtype=int):
    ...
    return cp.asarray(cp.nonzero(voxel_data), dtype)
...
def sparse_to_dense(voxel_data, dims, dtype=bool):
    ...
    if cp.isscalar(dims):
        dims = [dims]*3
    dims = cp.atleast_2d(dims).T
    # truncate to integers
    xyz = voxel_data.astype(int)
    # discard voxels that fall outside dims
    valid_ix = ~cp.any((xyz < 0) | (xyz >= dims), 0)
    xyz = xyz[:,valid_ix]
    out = cp.zeros(dims.flatten(), dtype=dtype)
    out[tuple(xyz)] = True
    return out
...
def write(voxel_model, fp):
    ...
    if voxel_model.axis_order=='xzy':
        ...
        voxels_flat = cp.transpose(dense_voxel_data, (0, 2, 1)).flatten()
