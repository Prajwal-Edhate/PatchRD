...
class MODEL_PATCH(object):
    def __init__(self, config):
        ...
        if torch.cuda.is_available():
            os.environ['CUDA_TOOLKIT_MEMCHECK'] = '1'
        ...
        self.patch_coord = cp.concatenate((cp.expand_dims(xx, -1), cp.expand_dims(yy, -1),cp.expand_dims(zz, -1)), -1)
        ...
        if os.path.exists("splits/"+self.data_content+"_train.txt"):
            ...
            if config.train_patch:
                for i in range(self.dataset_len):
                    ...
                    self.crop_locs.append(c_locs.get())

                    self.shape_names.append(self.dataset_names[self.names[i]])

                    if self.gua_filter:
                        tmp_input = cp.asarray(gaussian_filter(cp.asnumpy(tmp_input).astype(np.float32), sigma=1))
                    ...
                    if img_y<4:
                        ...
                        self.imgout_0[img_y*self.real_size:(img_y+1)*self.real_size,img_x*self.real_size:(img_x+1)*self.real_size] = self.voxel_renderer.render_img(tmpvox, self.sampling_threshold, self.render_view_id)
                cv2.imwrite(self.sample_dir+"/a_content_%s.png"%(self.mode), cp.asnumpy(self.imgout_0))
        else:
        ...
        if self.pred_coarse:
            ...
            if self.data_content =='content_chair':
                ...
                checkpoint = torch.load('checkpoint_complete/content_chair_complete/coarse_comp/checkpoint-100.pth')
            elif self.data_content=='content_boat':
                self.dis_thres=0.12
                checkpoint = torch.load('checkpoint_complete/content_boat_complete/coarse_comp/checkpoint-100.pth')
            elif self.data_content=='content_lamp':
                self.dis_thres=0.04
                checkpoint = torch.load('checkpoint_complete/content_lamp_complete/coarse_comp/checkpoint-100.pth')
                ...
                checkpoint = torch.load('checkpoint_complete/content_cabinet_complete/coarse_comp/checkpoint-100.pth')
            elif self.data_content=='content_plane':
                self.dis_thres = 0.045
                checkpoint = torch.load('checkpoint_complete/content_plane_complete/coarse_comp/checkpoint-100.pth')
            elif self.data_content=='content_car':
                self.dis_thres=0.05
                checkpoint = torch.load('checkpoint_complete/content_car_complete/coarse_comp/checkpoint-100.pth') #
            elif self.data_content=='content_table':
                self.dis_thres = 0.045
                checkpoint = torch.load('checkpoint_complete/content_table_complete/coarse_comp/checkpoint-100.pth')
    ...
    def random_crop(self, vox, crop_size=20, c_range=20, prob=None, return_locs=False ):
        vox = cp.asarray(vox)
        vx, vy, vz = vox.shape
        edges = cp.asarray(sobel(cp.asnumpy(vox)))
        edges = edges.astype(cp.float32)/255.0
    ...
    def reshape_to_patch_size(self, v1, v2):
        ...
        new_v1 = torch.from_numpy(cp.asnumpy(new_v1,)).unsqueeze(0).unsqueeze(0)

        new_v1 = F.pad(new_v1, tuple(cp.asnumpy(padding).astype(np.int8)))

        return cp.asarray(new_v1)[0,0]
    ...
    def filter_feat(self, feat, mask):
        ...
        yy, xx, zz = cp.meshgrid(y_,x_,z_)
        feat_coord = cp.concatenate((cp.expand_dims(xx, -1), cp.expand_dims(yy, -1),cp.expand_dims(zz, -1)), -1)
        filtered_feat = feats[mask[0,0]>0]
        filtered_coord = feat_coord[cp.asarray(mask[0,0])>0]
    ...
    def distance_retrieve(self, c_feat, d_feat, K=20,sigma=1.0, c_max_num=200, d_max_num=200):
        ...
        c_ids = cp.array(sorted(cp.random.randint(0, c_feat.shape[0], c_num)))
        d_ids = cp.array(sorted(cp.random.randint(0, d_feat.shape[0], d_num)))

        c_feat_chosen =  c_feat[c_ids,:].detach()
        d_feat_chosen =  d_feat[d_ids,:].detach()
        ...
        for i in range(c_num):
            ...
            tmp_d_ids = cp.random.choice(cp.arange(d_num), size=K, p=dis_prob[i])
            tmp_d_ids = tmp_d_ids.astype(cp.int32)
            out_ids[i] = d_ids[tmp_d_ids]
        return c_ids, out_ids.astype(cp.int32)

    def random_retrieve(self,num_c, num_d, K=20, c_max_num=200,):
        c_num = min(num_c, c_max_num)
        c_ids = sorted(cp.random.randint(0, num_c, c_num))
        d_ids = cp.zeros((c_num, K))
        for i in range(c_num):
            d_ids[i] = cp.random.randint(0,num_d, size=K)
        return c_ids, d_ids

    def compute_feat_dis(self,c_feats_less, d_feats_less, re_c_ids, re_d_ids):
        re_c_ids = np.array([cp.asnumpy(arr) for arr in re_c_ids])
        re_d_ids = np.array([cp.asnumpy(arr) for arr in re_d_ids])
        #print("re_c_ids",type(re_c_ids),re_c_ids)
        #print("re_c_ids",type(re_d_ids),re_d_ids)
        c_feats_less = c_feats_less[re_c_ids]
        out_dis = torch.zeros(re_d_ids.shape).to(self.device)
        #print(out_dis,type(out_dis))
    ...
    def compute_iou(self, recons, gt, thresh=0.4):
        x = (recons>thresh).astype(cp.float32)

        y = (gt>thresh).astype(cp.float32)

        intersection = cp.sum(x*y)
        union = cp.sum(x) + cp.sum(y) - cp.sum(x*y)
    ...
    def get_ids_with_gt(self, c_locs, total_d_locs):
         ...
         valid_ids = []
         for i in range(c_num):
             dif = cp.abs(total_d_locs - c_locs[i,]).sum(1).min()
             if dif==0:
                 valid_ids.append(i)
         return valid_ids
        ...
        for epoch in range(self.start_epoch,  training_epoch):
            ...
            for idx in range(self.dataset_len):
                ...
                if not self.pred_coarse:
                    input_in = torch.from_numpy(cp.asnumpy(self.input_content)[dxb]).to(self.device).unsqueeze(0).unsqueeze(0).float()
                    mask_in =  torch.from_numpy(cp.asnumpy(self.mask_content)[dxb]).to(self.device).unsqueeze(0).unsqueeze(0).float()

                gt_in = torch.from_numpy(cp.asnumpy(self.gt_content)[dxb]).to(self.device).unsqueeze(0).unsqueeze(0).float()
                ...
                if self.pred_coarse:
                    if self.mode=='test': 
                        ...
                        partial_tmp, partial_mask_tmp = self.random_crop(self.gt_content[dxb],crop_size=self.csize, \
                                c_range=self.c_range)
                        if self.gua_filter:
                            partial_tmp = cp.asarray(gaussian_filter(cp.asnumpy(partial_tmp).astype(np.float32), sigma=1))
                    
                    partial_tmp_in = torch.from_numpy(cp.asnumpy(partial_tmp)).to(self.device).unsqueeze(0).unsqueeze(0).float()
                ...
                #print("c_locs",c_locs,type(c_locs))
                d_feats_less, d_locs = self.filter_feat(d_feat, mask_partial_in)
                #print("c_locs",d_locs,type(d_locs))
                ...
                for top_num in self.top_nums:
                    if self.dump_deform and (epoch==self.start_epoch or epoch%20==0):
                        ...
                        if self.pred_coarse:
                            coarse_shape = cp.asarray(input_in)[0,0]
                        else:
                            coarse_shape = 0

                        np.savez(os.path.join(out_path, 'dump_deform_wc_top%d_%d.npz'%(top_num, epoch)), re_ids=cp.asnumpy(out_ids), \
                                re_dis=cp.asnumpy(out_dis), re_trans=cp.asnumpy(out_trans), re_ids_c=cp.asnumpy(out_ids_c), re_dis_c=cp.asnumpy(out_dis_c), \
                                re_trans_c=cp.asnumpy(out_trans_c), \
                                coarse_shape=cp.asnumpy(coarse_shape),  crop_locs=cp.asnumpy(crop_locs))
                    ...
                    if self.mode=='test' and (epoch==self.start_epoch or epoch%20==0):
                        ...
                        if self.pred_coarse:
                            imgout_0 = cp.full([self.real_size, self.real_size*7], 255, cp.uint8)
                        else:
                            imgout_0 = cp.full([self.real_size, self.real_size*5], 255, cp.uint8)
                        ...
                        if self.pred_coarse:
                            ...
                            tmpvox = self.recover_voxel(cp.asarray(input_in)[0][0], xmin,xmax,ymin,ymax,zmin,zmax)
                        ...
                        tmpvox = self.recover_voxel(cp.asarray(partial_in)[0][0],xmin,xmax,ymin,ymax,zmin,zmax)
                        imgout_0[:,self.real_size:self.real_size*2] = self.voxel_renderer.render_img(tmpvox, self.sampling_threshold,\
                                self.render_view_id)

                        tmpvox = self.recover_voxel(cp.asarray(gt_in)[0][0],xmin,xmax,ymin,ymax,zmin,zmax)
                        ...
                        iou = self.compute_iou(out_vox, cp.asarray(gt_in)[0][0],thresh=self.sampling_threshold)
                        total_iou += iou
                ...
                gt_dis = np.array([cp.asnumpy(arr) for arr in gt_dis])
                #print("gt_dis",type(gt_dis), gt_dis, gt_dis.mean(), gt_dis.min(), gt_dis.max())

                gt_dis = gt_dis*(gt_dis<self.dis_thres) + 10*gt_dis*(gt_dis>=self.dis_thres)
                gt_dis = torch.from_numpy(gt_dis).float().to(self.device)

                same_d_ids = cp.arange(d_locs.shape[0]).reshape(-1, 1)
                same_c_ids = cp.zeros(same_d_ids.shape[0])

                same_mask = cp.ones(d_locs.shape[0])

                for i in range(same_d_ids.shape[0]):
                    loc_dif = cp.sum(cp.square((c_locs - d_locs[i])), axis=1)
                    if loc_dif.min()>0:
                        same_mask[i]=0
                        continue

                    same_c_ids[i] = cp.where(loc_dif==0)[0][0]
                same_mask = same_mask.astype(cp.bool)
                same_c_ids = same_c_ids.astype(cp.int32)

                same_c_ids = same_c_ids[same_mask.get()]
                same_d_ids = same_d_ids[same_mask.get()]
                #print("same_c_ids",same_d_ids,type(same_d_ids))

                same_gt_dis = self.get_icp_dis(gt_in, partial_in,input_in, c_locs, d_locs, same_c_ids, same_d_ids, \
                                target_pt_num=self.pt_num, check=False)
                same_gt_dis = np.array([cp.asnumpy(arr) for arr in same_gt_dis])
                #print('same_gt_dis',type(same_gt_dis), same_gt_dis, same_gt_dis.mean(), same_gt_dis.min(), same_gt_dis.max())
            ...
            if epoch % 5==0 and self.mode=='train':
                ...
                cv2.imwrite(self.sample_dir+"/%s_%d_%.4f.png"%(self.mode, epoch, dis), cp.asnumpy(imgout_0))
    ...
    def get_ids_from_locs(self, locs, total_locs, or_ids):
        out_ids = cp.zeros(locs.shape[0])
        for i in range(locs.shape[0]):
            loc_dif = cp.sum(cp.abs(cp.expand_dims(locs[i],0) - total_locs), 1)
            if loc_dif.min()>0:
                out_ids[i] = or_ids[i]
            else:
                out_ids[i] = cp.where(loc_dif==0)[0][0]
        return out_ids
    ...
    def get_icp_dis(self, gt_in, partial_in,coarse_in, c_locs, d_locs, re_c_ids,re_d_ids, target_pt_num=768, check=False, return_vox=False):
        ...
        gt_np = cp.asarray(gt_in)[0,0]
        partial_np = cp.asarray(partial_in)[0,0]
        ...
        all_d_ids = cp.asarray(np.unique(re_d_ids).astype(np.int32))
        d_starts = d_locs[all_d_ids]*self.stride

        if return_vox==True:
            coarse_np = cp.asarray(coarse_in)[0,0]
            real_d_starts = d_locs[re_d_ids[:,0]]*self.stride


        target_points = cp.zeros((c_starts.shape[0], target_pt_num, 3),)
        source_points_all = cp.zeros((d_starts.shape[0], target_pt_num, 3),)
        source_points_all_re = cp.zeros((d_starts.shape[0], target_pt_num, 3),)

        if return_vox==True:
            or_c_vox = cp.zeros((c_starts.shape[0], self.patch_size, self.patch_size, self.patch_size))
            or_d_vox = cp.zeros((c_starts.shape[0], self.patch_size, self.patch_size, self.patch_size))
            gt_vox = cp.zeros((c_starts.shape[0], self.patch_size, self.patch_size, self.patch_size))
        ...
        for i in range(d_starts.shape[0]):
            ...
            source_points_all_re[i] = source_points_all[i]*cp.array([1.0,1.0,-1.0])

        out_dis = cp.ones((len(re_c_ids), k))

        for i in range(k):
            source_points = cp.zeros((c_starts.shape[0], target_pt_num, 3))
            source_points_re = cp.zeros((c_starts.shape[0], target_pt_num, 3))
            for j in range(c_starts.shape[0]):
                # print(re_d_ids[j,i], np.where(all_d_ids==re_d_ids[j,i])[0][0])
                source_pt_idx = cp.where(all_d_ids==re_d_ids[j,i])[0][0]
                source_points[j] =  source_points_all[source_pt_idx.get()]
                source_points_re[j] = source_points_all_re[source_pt_idx.get()]
            ...
            trans_i = np.array([cp.asnumpy(arr) for arr in trans_i])
            trans_i_re = np.array([cp.asnumpy(arr) for arr in trans_i_re])
            out_dis[:,i] = cp.minimum(dis_i, dis_i_re)


            if check==True:
                patch_renderer = voxel_renderer(self.patch_size)
                imgout_0 = cp.full([self.patch_size, self.patch_size*3], 255, cp.uint8)
                ...
                if not os.path.exists(out_path):
                    os.makedirs(out_path)
                d_starts = d_locs[re_d_ids[:,0].astype(cp.int32)]*self.stride
                for j in range(c_starts.shape[0]):
                    ...
                    cv2.imwrite(os.path.join(out_path, "%d_%.4f.png"%(j, im_dis)), cp.asnumpy(imgout_0))
        return out_dis

    def get_icp_dis_from_patches(self, d_patches, gt_patches,target_pt_num=768, ):
        target_points = cp.zeros((gt_patches.shape[0], target_pt_num, 3),)
        source_points = cp.zeros((d_patches.shape[0], target_pt_num, 3),)
        source_points_re = cp.zeros((d_patches.shape[0], target_pt_num, 3),)
        ...
        for i in range(d_patches.shape[0]):
            ...
            source_points_re[i] = source_points[i]*cp.array([1.0,1.0,-1.0])
        dis_i, trans_i = self.icp_optim(source_points, target_points,)
        dis_i_re, trans_i_re = self.icp_optim(source_points_re, target_points,)
        out_dis = cp.minimum(dis_i, dis_i_re)
        return out_dis
    ...
    def icp_optim(self, source, target,icp_learning_rate = 0.5,icp_n_iters = 20, device='cuda:0'):
        ...
        source, target: [p,p,p,]
        """
        source = np.array([cp.asnumpy(arr) for arr in source])
        target = np.array([cp.asnumpy(arr) for arr in target])
        ...
        return cp.asarray(dis.data), cp.asarray(param_init.detach())
    ...
    def retrieve_to_full_shape(self, c_feat, c_locs, d_feat, d_locs, gt_in, partial_in,coarse_in,idx,  coarse=False, dump_patch=False, sigma=1, target_pt_num=768):
        ...
        c_locs_mask = c_locs_mask2.astype(cp.int32)*c_locs_mask
        ...
        re_ids = cp.argmin(dis_matrix, axis=1)
        d_locs_final = d_locs[re_ids]

        source_points = cp.zeros((c_num, target_pt_num, 3))
        source_points_re = cp.zeros((c_num, target_pt_num, 3))
        target_points = cp.zeros((c_num, target_pt_num, 3))
        out_vox = cp.zeros(gt_np.shape)
        out_vox_or = cp.zeros(gt_np.shape)
        ...
        for i in range(c_num):
            ...
            if coarse:
                ...
                down_rate = self.output_size//self.input_size
                vox_tensor = torch.from_numpy(cp.asnumpy(vox_partial)).float().unsqueeze(0).unsqueeze(0).cuda()
            ...
            vox_partial_re = cp.flip(vox_partial, 2)
        ...
        trans = np.array([cp.asnumpy(arr) for arr in trans])
        
        dis_re, trans_re = self.icp_optim(source_points_re, target_points,)
        trans_re = np.array([cp.asnumpy(arr) for arr in trans_re])
    ...
    def retrieve_to_full_shape_topk(self, c_feat, c_locs, d_feat, d_locs, gt_in, partial_in, coarse_in, idx,  return_k=False, \
        ...
        #c_feat and d_featis tensor
        c_locs = np.array([cp.asnumpy(arr) for arr in c_locs])
        d_locs = np.array([cp.asnumpy(arr) for arr in d_locs])
        ...
        total_dis = cp.zeros((c_num,top_num))
        total_trans = cp.zeros((c_num, top_num, 4))
        total_flip = cp.zeros((c_num,top_num))
        ...
        for k_i in range(top_num):
            ...
            for i in range(c_num):
                ...
                vox_partial = partial_np[d_starts[i, 0]:d_starts[i, 0]+self.patch_size, \
                    d_starts[i, 1]:d_starts[i, 1]+self.patch_size, d_starts[i, 2]:d_starts[i, 2]+self.patch_size]
                vox_partial = np.array([cp.asnumpy(arr) for arr in vox_partial])
                if coarse:
                    vox_gt = coarse_np[c_starts[i, 0]:c_starts[i, 0]+self.patch_size, \
                        c_starts[i, 1]:c_starts[i, 1]+self.patch_size, c_starts[i, 2]:c_starts[i, 2]+self.patch_size]
                    ...
                    if self.input_size==64:
                        ...
                        input_vox = F.max_pool3d(vox_tensor, kernel_size = down_rate, stride = 1, padding = down_rate//2)
                        input_vox = input_vox[:,:,:-1,:-1,:-1]
                    vox_partial = cp.asarray(input_vox)[0,0]
        ...
        d_ids_tmp = cp.argmin(total_dis, axis=1)
        out_dis = cp.zeros(c_num)
        ...
        for i in range(c_num):
            ...
            loc_mask =loc_mask2.astype(cp.int32)*loc_mask
    ...
    def pad_mask(self, v1, v2,mode='replicate'):
        ...
        new_v1 = torch.from_numpy(cp.asnumpy(v1)).float().to(self.device).unsqueeze(0).unsqueeze(0)
    ...
    def move_vox(self, vox, shift):
        print("vox",vox,type(vox))
        vox = cp.asnumpy(cp.asarray(vox))
        ...
        span = self.patch_size - cp.abs(shift)
    ...
    def move_vox_torch(self, vox, shift):
        vox = cp.asarray(vox)
        out_vox = torch.zeros(cp.asnumpy(vox).shape).to(self.device)
        start_1 = cp.maximum(shift, cp.zeros(shift.shape)).astype(cp.int32)
        start_2 = cp.maximum(cp.zeros(shift.shape), -shift).astype(cp.int32)

        span = self.patch_size - cp.abs(shift)
        end_1= cp.minimum([start_1[0]+span[0], start_1[1]+span[1], start_1[2]+span[2]], \
             [self.patch_size, self.patch_size, self.patch_size]).astype(cp.int32)
        r_span = cp.array([end_1[0]-start_1[0], end_1[1]-start_1[1], end_1[2]-start_1[2],])
    ...
    def volume_to_point_cloud_to_num(self,vol, coord, target_num=768):
        ...
        pt_mask = (vol>self.pt_sampling_threshold).astype(cp.int32)


        or_points= coord[pt_mask.astype(cp.bool)].astype(cp.float32)
        num_points = pt_mask.sum()
        # print('num_points', num_points)

        if num_points==0:
            return cp.zeros((target_num,3))
        if num_points>=target_num:
            point_ids = cp.random.randint(0,target_num, size=target_num)
            points = or_points[point_ids.get()]
        else:
            points=[]
            points.append(or_points.get())
            up_rate = target_num//num_points
            for i in range(up_rate.get()):
                #print(num_points,type(num_points))
                pert_points = or_points + 0.*cp.random.rand(num_points.get(), 3)
                points.append(pert_points.get())
            points = cp.array(cp.asarray(points)).reshape((up_rate.get()+1)*num_points.get(), 3)
            point_ids = cp.random.randint(0,points.shape[0], size=target_num)
            points = points[point_ids.get()]
        # [-1, 1]
    ...
    def patch_to_xyzrgb(self, patch, c_loc, d_loc):
        ...
        if num_points==0:
            return cp.zeros((0,6))
        points = points + cp.expand_dims(c_loc, axis=0)
        colors = cp.repeat(cp.expand_dims(d_loc, 0).get(), num_points.get(), 0)
        xyzrgb = cp.concatenate([points, colors], axis=1)
        return xyzrgb


    def volume_to_point_cloud_new(self, vol,coord, thres=0.3,target_num=100):
        vol = cp.asarray(vol)
        pt_mask = (vol>thres).astype(cp.int32)

        padded_mask = cp.pad(pt_mask, ((1,1), (1,1), (1,1)))
    ...
    def vol_to_point_cloud2(self, vol, vsize, radius=0.5):
        vol = cp.asarray(vol)
        pt_mask = (vol>self.sampling_threshold).astype(cp.int32)
        or_points= self.shape_coord[pt_mask.astype(cp.bool)].astype(cp.float32)
        num_points = pt_mask.sum()
        if num_points==0:
            return cp.zeros((0,3))
